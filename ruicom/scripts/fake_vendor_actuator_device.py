...
class FakeVendorActuatorDevice:
    def __init__(self):
        rospy.init_node('fake_vendor_actuator_device', anonymous=False)
        ...
        self.result_topic = str(rospy.get_param('~result_topic', 'recon/platform/vendor/actuator_result_raw')).strip() or 'recon/platform/vendor/actuator_result_raw'
        ...
        self.pub = rospy.Publisher(self.result_topic, String, queue_size=20)
        self.sub = rospy.Subscriber(self.command_topic, String, self._cb, queue_size=20)
    def _emit(self, payload):
        msg = {'command_id': str(payload.get('command_id', '')).strip(), 'status': self.status, 'stamp': rospy.get_time(), 'details': {'driver': 'fake_vendor_actuator_device', 'action_type': str(payload.get('action_type', '')).strip()}}
