...
class FakeMoveBaseActionServer:
    ...
    def __init__(self):
        rospy.init_node('fake_move_base_action_server', anonymous=False)
        ...
        rospy.loginfo(
            'fake_move_base_action_server started. action_name=%s pose_topic=%s bootstrap_pose=%s',
            self.action_name,
            self.pose_topic,
    ...
    def _execute(self, goal) -> None:
        ...
        while not rospy.is_shutdown():
            if self.server.is_preempt_requested():
                self.server.set_preempted(text='preempted by fake server')
                return
            ...
            if elapsed >= self.goal_completion_delay_sec:
                if not pose_published:
                    self._publish_pose_for_goal(goal)
                self.server.set_succeeded(MoveBaseResult(), text='fake goal succeeded')
                return
            rate.sleep()
