...
class SyntheticScenePublisher:
    ...
    def __init__(self):
        ...
        self.classes = [str(item).strip() for item in rospy.get_param('~classes', ['friendly', 'enemy', 'hostage']) if str(item).strip()]
        raw_palette = rospy.get_param('~class_palette', {})
        self.class_palette = {}
        if isinstance(raw_palette, dict):
            for class_name, value in raw_palette.items():
                if isinstance(value, (list, tuple)) and len(value) == 3:
                    self.class_palette[str(class_name).strip()] = tuple(int(channel) for channel in value)
        if self.width <= 0:
        ...
        if not isinstance(self.named_regions, list):
            raise ValueError('named_regions must be a list')
        if not self.classes:
    ...
    def _class_color(self, class_name: str):
        if class_name in self.class_palette:
            return self.class_palette[class_name]
        if class_name == 'friendly':
            return (0, 0, 255)
        if class_name == 'enemy':
            return (255, 0, 0)
        if class_name == 'hostage':
            return (20, 20, 20)
        return (80, 80, 80)
