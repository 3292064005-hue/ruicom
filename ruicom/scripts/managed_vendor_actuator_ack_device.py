...
class ManagedVendorActuatorAckDevice:
    """Managed actuator ack adapter for strict deploy validation.

    This node is a controlled device-layer stand-in: it subscribes to raw vendor
    actuator commands and emits actuator-device acknowledgements on the same
    contract topics expected by vendor_actuator_device_node when operating in
    external_ack mode.
    """
    def __init__(self):
        rospy.init_node('managed_vendor_actuator_ack_device', anonymous=False)
        ...
        rospy.Subscriber(self.command_topic, String, self._cb, queue_size=20)
    def _emit(self, payload):
        msg = {
            ...
            'details': {
                'device': 'managed_vendor_actuator_ack_device',
...
if __name__ == '__main__':
    ManagedVendorActuatorAckDevice().spin()
